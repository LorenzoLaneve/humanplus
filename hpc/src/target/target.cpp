...
#define FRelocModel llvm::Optional<llvm::Reloc::Model>()// FIXME: -relocation-model by flag
...
/*
    for (llvm::Function &func : wrapper.getModule()) {
        ...
        llvm::AttributeList attribs = func.getAttributes(), nattribs;
        
        if (!CPU.empty())
            nattribs = nattribs.addAttribute(context, llvm::AttributeList::FunctionIndex,
                                             "target-cpu", CPU);
                ...
                if (auto *fcall = llvm::dyn_cast<llvm::CallInst>(&I))
                    if (const auto *F = fcall->getCalledFunction())
                        if (F->getIntrinsicID() == llvm::Intrinsic::debugtrap ||
                            ...
                            fcall->addAttribute(context, llvm::AttributeList::FunctionIndex,
                                                "trap-func-name", TrapFuncName);
        
        nattribs = attribs.addAttributes(context, llvm::AttributeList::FunctionIndex, nattribs);
